...
import { Mail, MessageSquare, ExternalLink, Handshake } from "lucide-react";
...
export function ContactCard({
  ...
  const visibility = normalizeEmailVisibility(
      ...
      <CardContent className="p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {user.socialLinks.github && (
              ...
              >
                <Badge
                  variant="outline"
                  className="text-[10px] border-white/10 text-zinc-400 hover:text-zinc-200 cursor-pointer gap-1"
                >
        ...
        {showMessageButton ? (
          <Link to={`/messages?to=${user.id}`} className="block">
            <Button
              className="w-full gap-2 bg-indigo-600 hover:bg-indigo-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.2)]"
              size="sm"
              data-testid={messageButtonTestId}
            >
