...
export function ContactCard({ user, showEmail = true, className }: ContactCardProps) {
  ...
  const visibility = user.privacySettings?.emailVisibility || "HIDDEN";
        ...
        {showEmail && visibility !== "HIDDEN" && displayEmail && (
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-zinc-300">
              {visibility === "FULL" ? displayEmail : maskEmail(displayEmail)}
            </span>
          </div>
        )}

        {showEmail && visibility === "HIDDEN" && (
          <p className="text-xs text-zinc-500 italic">Email is set to private</p>
        )}
